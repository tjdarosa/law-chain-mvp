...
describe('Asset Transfer Besic REST API', () => {
    ...
    describe('/live', () => {
        it('GET should respond with 200 OK json', async () => {
            ...
            const mockCollectingOfficerQsccContract = mock<Contract>();
            mockCollectingOfficerQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdCollectingOfficer] = {
                qsccContract: mockCollectingOfficerQsccContract,
            };

            const mockEvidenceCustodianQsccContract = mock<Contract>();
            mockEvidenceCustodianQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdEvidenceCustodian] = {
                qsccContract: mockEvidenceCustodianQsccContract,
            };
    ...
    describe('/api/assets', () => {
        ...
        it('GET should respond with an empty json array when there are no assets', async () => {
            ...
            const response = await request(app)
                .get('/api/assets')
                .set('X-Api-Key', 'COLLECTINGOFFICERMOCKAPIKEY');
            expect(response.statusCode).toEqual(200);
            expect(response.header).toHaveProperty(
                'content-type',
                'application/json; charset=utf-8'
