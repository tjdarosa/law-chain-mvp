...
export const fabricAPIKeyStrategy: HeaderAPIKeyStrategy =
    new HeaderAPIKeyStrategy(
        ...
        function (apikey, done) {
            ...
            if (apikey === config.collectingofficerApiKey) {
                const user = config.mspIdCollectingOfficer;
                logger.debug('User set to %s', user);
                done(null, user);
            } else if (apikey === config.evidencecustodianApiKey) {
                const user = config.mspIdEvidenceCustodian;
                logger.debug('User set to %s', user);
                done(null, user);
            } else {
