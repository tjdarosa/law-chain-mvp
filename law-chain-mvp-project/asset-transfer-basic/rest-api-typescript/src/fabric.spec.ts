...
describe('Fabric', () => {
    describe('createWallet', () => {
        it('creates a wallet containing identities for both orgs', async () => {
            ...
            expect(await wallet.list()).toStrictEqual(['CollectingOfficerMSP', 'EvidenceCustodianMSP']);
        });
    });

    describe('createGateway', () => {
        it('creates a Gateway and connects using the provided arguments', async () => {
            const connectionProfile = config.connectionProfileCollectingOfficer;
            const identity = config.mspIdCollectingOfficer;
