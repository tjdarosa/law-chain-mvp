...
/**
 * Creates an in memory wallet to hold credentials for an CollectingOfficer and EvidenceCustodian user
 *
...
export const createWallet = async (): Promise<Wallet> => {
    ...
    const collectingofficerIdentity = {
        credentials: {
            certificate: config.certificateCollectingOfficer,
            privateKey: config.privateKeyCollectingOfficer,
        },
        mspId: config.mspIdCollectingOfficer,
        type: 'X.509',
    };

    await wallet.put(config.mspIdCollectingOfficer, collectingofficerIdentity);

    const evidencecustodianIdentity = {
        credentials: {
            certificate: config.certificateEvidenceCustodian,
            privateKey: config.privateKeyEvidenceCustodian,
        },
        mspId: config.mspIdEvidenceCustodian,
        type: 'X.509',
    };

    await wallet.put(config.mspIdEvidenceCustodian, evidencecustodianIdentity);
