...
test network. The channel will have two members, CollectingOfficer and EvidenceCustodian, that will
...
needs to be endorsed by peers that belong to CollectingOfficer and EvidenceCustodian. When the asset is
...
transactions. Because the asset is owned by CollectingOfficer, any future updates to the asset
need to be endorsed by the CollectingOfficer peer.
- Update the asset by only endorsing with CollectingOfficer, this will use the state-based
endorsement policy applied to the asset when it was created by the chaincode.
- Transfer the asset to EvidenceCustodian. During the execution of the transfer transaction,
the chaincode will create a new state-based endorsement policy that reflects
the new asset owner for the asset.
- Update the asset once more, this time with EvidenceCustodian as the owner. Because the
state-based endorsement policy has been updated, this transaction only needs
to be endorsed by EvidenceCustodian.
...
You can use the test network script to deploy the smart contract to the channel that was just created. The script uses the Fabric chaincode lifecycle to deploy the smart contract to the channel. We will use the default chaincode level endorsement policy used by the Fabric chaincode lifecycle, which requires an endorsement from a majority of channel members. In our case, this will require that both CollectingOfficer and EvidenceCustodian endorse a transaction (2 of 2). Deploy the smart contract to `mychannel` using the following command:
```
./network.sh deployCC -ccn sbe -ccp ../asset-transfer-sbe/chaincode-typescript/ -ccl typescript
```

Set the following environment variables to interact with the network as a user from CollectingOfficer:
...
```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/collectingofficer.example.com/peers/peer0.collectingofficer.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/evidencecustodian.example.com/peers/peer0.evidencecustodian.example.com/tls/ca.crt" -c '{"function":"CreateAsset","Args":["asset1","100","CollectingOfficerUser1"]}'
```
The create transaction needs to target both peers from CollectingOfficer and EvidenceCustodian to meet the chaincode endorsement policy. The chaincode will read the MSP ID of the client user submitting the transaction and assign that organization as the asset owner. As a result, the asset will initially be owned by CollectingOfficer.

You can query the asset using with the following command:
```
peer chaincode query -C mychannel -n sbe -c '{"Args":["ReadAsset","asset1"]}'
```
The result is a new asset owned by CollectingOfficer, identified using the CollectingOfficer MSP ID `CollectingOfficerMSP`:
`{"ID":"asset1","Value":100,"Owner":"CollectingOfficerUser1","OwnerOrg":"CollectingOfficerMSP"}`

In addition to creating the asset, the `CreateAsset` function also sets a state-based endorsement policy for the asset. Only a peer of the asset owner, can successfully endorse an asset update. To demonstrate the key-level endorsement policy, lets try to update the asset while targeting the EvidenceCustodian peer:
```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/evidencecustodian.example.com/peers/peer0.evidencecustodian.example.com/tls/ca.crt" -c '{"function":"UpdateAsset","Args":["asset1","200"]}'
```
...
If we attempt to update the asset with an endorsement from the CollectingOfficer peer, the update succeeds:
```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/collectingofficer.example.com/peers/peer0.collectingofficer.example.com/tls/ca.crt" -c '{"function":"UpdateAsset","Args":["asset1","200"]}'
```
...
```
{"ID":"asset1","Value":200,"Owner":"CollectingOfficerUser1","OwnerOrg":"CollectingOfficerMSP"}
```

Now that we have tested the asset key-level endorsement policy, we can transfer the asset to EvidenceCustodian. Run the following command to transfer the asset from CollectingOfficer to EvidenceCustodian. This time the EvidenceCustodian MSP ID is provided as a transaction input. The `TransferAsset` function will update the endorsement policy to specify that only a peer of the new owner can update the asset. Note that this command targets the CollectingOfficer peer.

```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/collectingofficer.example.com/peers/peer0.collectingofficer.example.com/tls/ca.crt" -c '{"function":"TransferAsset","Args":["asset1","EvidenceCustodianUser1","EvidenceCustodianMSP"]}'
```

We can query the asset to see that the owner has been updated from CollectingOfficer to EvidenceCustodian:
```
peer chaincode query -C mychannel -n sbe -c '{"Args":["ReadAsset","asset1"]}'
```

The owning organization is now EvidenceCustodian:
```
{"ID":"asset1","Value":200,"Owner":"EvidenceCustodianUser1","OwnerOrg":"EvidenceCustodianMSP"}
```

EvidenceCustodian now needs to endorse any asset updates. Run the following command to try to update the asset with an endorsement from the CollectingOfficer peer:
```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/collectingofficer.example.com/peers/peer0.collectingofficer.example.com/tls/ca.crt" -c '{"function":"UpdateAsset","Args":["asset1","300"]}'
```
...
Now try to update the asset with an endorsement from the EvidenceCustodian peer:
```
peer chaincode invoke -o localhost:7050 --waitForEvent --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n sbe --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/evidencecustodian.example.com/peers/peer0.evidencecustodian.example.com/tls/ca.crt" -c '{"function":"UpdateAsset","Args":["asset1","300"]}'
```
