...
func setAssetStateBasedEndorsement(ctx contractapi.TransactionContextInterface, assetId string, mspids []string, auditor bool) error {
	...
	if !auditor {
		...
		// AND(auditor, OR(CollectingOfficer, EvidenceCustodian))
		...
		// equivilent to OR(AND(CollectingOfficer, EvidenceCustodian),AND(auditor, OR(CollectingOfficer, EvidenceCustodian)))
		policy := &common.SignaturePolicyEnvelope{
			Version: 0,
			Rule: &common.SignaturePolicy{
				Type: &common.SignaturePolicy_NOutOf_{
