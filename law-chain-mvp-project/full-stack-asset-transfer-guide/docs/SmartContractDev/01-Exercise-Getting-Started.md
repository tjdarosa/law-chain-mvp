...
It also includes credentials for an `collectingofficer` organization, which will be used to run the peer. We'll use an `collectingofficer` admin user when interacting with the environment.
...
A file `collectingofficeradmin.env` is written out that contains the environment variables needed to run applications _as the collectingofficer admin identity_. A second organization is created, with a `evidencecustodianadmin.env` - this is for later exercises and is not needed at the moment.
...
`collectingofficeradmin.id` contains the credentials for submitting transactions from an collectingofficer admin.
`collectingofficercaadmin.id` contains the credentials for creating additional identities in the collectingofficer Certificate Authority (CA).
...
You'll see the contents of collectingofficeradmin.id:
...
```bash
peer lifecycle chaincode approveformyorg -law-channel -o $ORDERER_ENDPOINT --name asset-transfer -v 0 --package-id $CHAINCODE_ID --sequence 2 --connTimeout 15s
peer lifecycle chaincode commit -law-channel -o $ORDERER_ENDPOINT --name asset-transfer -v 0 --sequence 2 --connTimeout 15s
```
