...
function network_up() {
  ...
  wait_for ibppeer collectingofficer-peer1
  wait_for ibppeer collectingofficer-peer2
  wait_for ibppeer evidencecustodian-peer1
  wait_for ibppeer evidencecustodian-peer2
...
function launch_network_CAs() {
  ...
  wait_for ibpca collectingofficer-ca
  wait_for ibpca evidencecustodian-ca
  ...
  export COLLECTINGOFFICER_CA_CERT=$(kubectl -n $NS get cm/collectingofficer-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)
  export EVIDENCECUSTODIAN_CA_CERT=$(kubectl -n $NS get cm/evidencecustodian-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)

  enroll_bootstrap_rcaadmin org0 rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin collectingofficer rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin evidencecustodian rcaadmin rcaadminpw
