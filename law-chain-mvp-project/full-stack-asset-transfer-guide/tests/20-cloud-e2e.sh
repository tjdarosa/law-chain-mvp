...
curl -s --cacert $WORKSHOP_CRYPTO/cas/collectingofficer-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-collectingofficer-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
curl -s --cacert $WORKSHOP_CRYPTO/cas/evidencecustodian-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-evidencecustodian-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
...
# collectingofficer-peer1 peer CLI context
...
# Try a few times with collectingofficer-peer1
export ENDPOINT=${WORKSHOP_NAMESPACE}-collectingofficer-peer1-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with collectingofficer-peer2
export ENDPOINT=${WORKSHOP_NAMESPACE}-collectingofficer-peer2-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with the gateway endpoint.  Connections will be distributed across collectingofficer-peer1 and collectingofficer-peer2
