...
```shell
kubectl -n test-network port-forward svc/collectingofficer-peer1 7051:7051 
```

2.  Add "mock DNS" records to /etc/hosts for TLS host validation: 
```shell
127.0.0.1 collectingofficer-peer1 
```

3.  Configure the gateway client to connect to `collectingofficer-peer1:7051`, or the kube TCP port forward.
