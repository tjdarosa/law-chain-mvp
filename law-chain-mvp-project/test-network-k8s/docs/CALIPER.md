...
```shell
cp <fabric-samples install dir>/test-network-k8s/build/fabric-rest-sample-config/HLF_CONNECTION_PROFILE_COLLECTINGOFFICER networks/fabric/connection-profile.json
```
...
```json
    "peers": {
        "collectingofficer-peers": {
            "url": "grpcs://collectingofficer-peer1.localho.st:443",
            "tlsCACerts": {
                "pem": <contents of pem file>"
            },
            "grpcOptions": {
                "ssl-target-name-override": "collectingofficer-peer1.localho.st",
                "hostnameOverride": "collectingofficer-peer1.localho.st"
            }
        }
    },
```
...
organizations:
  - mspid: CollectingOfficerMSP
    ...
    identities:
      ...
      - name: 'User1'
        clientPrivateKey:
          path: '<fabric-samples install absolute dir>/test-network-k8s/build/enrollments/collectingofficer/users/collectingofficeradmin/msp/keystore/key.pem'

        clientSignedCert:
          path: '<fabric-samples install absolute dir>/test-network-k8s/build/enrollments/collectingofficer/users/collectingofficeradmin/msp/signcerts/cert.pem'
