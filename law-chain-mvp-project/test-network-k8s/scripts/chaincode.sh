...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for collectingofficer"
    install_chaincode $@
    log "🏁 - Chaincode is installed"

  elif [ "${COMMAND}" == "approve" ]; then
    log "Approving chaincode for collectingofficer"
    approve_chaincode $@
    log "🏁 - Chaincode is approved"

  elif [ "${COMMAND}" == "commit" ]; then
    log "Committing chaincode for collectingofficer"
    commit_chaincode $@
...
function query_chaincode() {
  ...
  export_peer_context collectingofficer peer1
...
function query_chaincode_metadata() {
  ...
  log ''
  log 'CollectingOfficer-Peer1:'
  export_peer_context collectingofficer peer1
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args

  log ''
  log 'CollectingOfficer-Peer2:'
  export_peer_context collectingofficer peer2
  peer chaincode query -n $cc_name -C $CHANNEL_NAME -c $args
}
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $COLLECTINGOFFICER_NS apply -f -

  kubectl -n $COLLECTINGOFFICER_NS rollout status deploy/${org}${peer}-ccaas-${cc_name}

  pop_fn
}

function launch_chaincode() {
  local org=collectingofficer
  local cc_name=$1
  local cc_id=$2
  local cc_image=$3
