...
function launch_peers() {
  ...
  apply_template kube/collectingofficer/collectingofficer-peer1.yaml $COLLECTINGOFFICER_NS
  apply_template kube/collectingofficer/collectingofficer-peer2.yaml $COLLECTINGOFFICER_NS
  apply_template kube/evidencecustodian/evidencecustodian-peer1.yaml $EVIDENCECUSTODIAN_NS
  apply_template kube/evidencecustodian/evidencecustodian-peer2.yaml $EVIDENCECUSTODIAN_NS

  kubectl -n $COLLECTINGOFFICER_NS rollout status deploy/collectingofficer-peer1
  kubectl -n $COLLECTINGOFFICER_NS rollout status deploy/collectingofficer-peer2
  kubectl -n $EVIDENCECUSTODIAN_NS rollout status deploy/evidencecustodian-peer1
  kubectl -n $EVIDENCECUSTODIAN_NS rollout status deploy/evidencecustodian-peer2
...
function create_local_MSP() {
  ...
  create_peer_local_MSP collectingofficer peer1 $COLLECTINGOFFICER_NS
  create_peer_local_MSP collectingofficer peer2 $COLLECTINGOFFICER_NS

  create_peer_local_MSP evidencecustodian peer1 $EVIDENCECUSTODIAN_NS
  create_peer_local_MSP evidencecustodian peer2 $EVIDENCECUSTODIAN_NS
...
function stop_services() {
  ...
  for ns in $ORG0_NS $COLLECTINGOFFICER_NS $EVIDENCECUSTODIAN_NS; do
...
function scrub_org_volumes() {
  ...
  for org in org0 collectingofficer evidencecustodian; do
