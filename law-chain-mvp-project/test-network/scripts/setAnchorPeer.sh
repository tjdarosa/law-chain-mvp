...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.collectingofficer.example.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.evidencecustodian.example.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.forensicanalyst.example.com"
    PORT=11051
  elif [ $ORG -eq 4 ]; then
    HOST="peer0.prosecutor.example.com"
    PORT=13051
  elif [ $ORG -eq 5 ]; then
    HOST="peer0.courtroompersonnel.example.com"
    PORT=15051
  else
