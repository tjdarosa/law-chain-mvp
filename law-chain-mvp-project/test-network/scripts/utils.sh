...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"law-channel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c law-channel -s couchdb "
    println "   network.sh up createChannel -bft -c law-channel -s couchdb "
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from CollectingOfficer and EvidenceCustodian"
