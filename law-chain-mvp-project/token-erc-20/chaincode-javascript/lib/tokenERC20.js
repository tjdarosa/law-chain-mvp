...
class TokenERC20Contract extends Contract {
    ...
    */
    ...
    async Initialize(ctx, name, symbol, decimals) {
        // Check minter authorization - this sample assumes CollectingOfficer is the central banker with privilege to set Options for these tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'CollectingOfficerMSP') {
    ...
    async Burn(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes CollectingOfficer is the central banker with privilege to burn tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'CollectingOfficerMSP') {
            throw new Error('client is not authorized to mint new tokens');
        }
